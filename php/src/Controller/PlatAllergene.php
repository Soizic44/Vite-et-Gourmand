...
#[Route('/api/plat-allergene', name: 'api_plat_allergene_')]
class PlatAllergeneController extends AbstractController
{
/* READ ALL avec GET /api/plat-allergene */
...
/* CREATE avec POST /api/plat-allergene*/
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        $stmtPlat->execute(['id' => $data['idPlat']]);
        ...
        $sqlAllergene = "SELECT * FROM allergene
                        WHERE idAllergene = :id";
        $stmtAllergene = $pdo->prepare($sqlAllergene);
        $stmtAllergene->execute(['id' => $data['idAllergene']]);

        if (!$stmtAllergene->fetch()) {
            return $this->json(['message' => 'Allergène inexistant'], 404);
        }

        // Vérification doublon
        $sqlDoublon = "SELECT * FROM platAllergene
                    WHERE idPlat = :plat
        ...
        // INSERT
        $sql = "INSERT INTO platAllergene(idPlat, idAllergene)
            VALUES(:plat, :allergene)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            'plat' => $data['idPlat'],
            'allergene' => $data['idAllergene']
    ...
    // DELETE avec DELETE /api/plat-allergene/{id}
    ...
    public function delete($id): JsonResponse
    {
        ...
        $sqlCheck = "SELECT * FROM platAllergene
